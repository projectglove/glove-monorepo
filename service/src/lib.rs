...
use client_interface::metadata::runtime_types::pallet_conviction_voting::vote::{
    AccountVote, Vote,
};
...
use client_interface::{
    account_to_subxt_multi_address, subscan, CallableSubstrateNetwork, ReferendumStatus,
    SubstrateNetwork,
};
use common::attestation::{AttestationBundle, AttestationBundleLocation};
use common::{
    AssignedBalance, Conviction, ExtrinsicLocation, GloveResult, SignedVoteRequest, VoteDirection,
    BASE_AYE, BASE_NAY,
};
...
fn to_account_vote(
    ...
    match direction {
        VoteDirection::Aye => AccountVote::Standard {
            vote: Vote(BASE_AYE + offset),
            balance,
        },
        VoteDirection::Nay => AccountVote::Standard {
            vote: Vote(BASE_NAY + offset),
            balance,
        },
        VoteDirection::Abstain => AccountVote::SplitAbstain {
            aye: 0,
            nay: 0,
            abstain: balance,
        },
...
impl GloveContext {
    ...
    pub async fn remove_vote_request(
        ...
        self.storage
            .remove_vote_request(poll_index, account)
            .await?;
        Ok(true)
    }
...
impl GloveState {
    ...
    pub async fn was_vote_added(&self, poll_index: u32) -> bool {
        self.get_poll_state_ref(poll_index)
            .await
            .read_access()
            .await
            .vote_added
