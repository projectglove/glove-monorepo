...
use aws_sdk_dynamodb::types::{AttributeValue, KeyType, ReturnValue, ScalarAttributeType, TableDescription, TableStatus};
...
impl DynamodbGloveStorage {
    pub async fn connect(table_name: String) -> anyhow::Result<Self> {
        ...
        let table_description = client.describe_table().table_name(&table_name).send().await?
            .table
            .ok_or_else(|| anyhow!("Table not found"))?;
        if table_description.table_status != Some(TableStatus::Active) {
            bail!("Table is not in active state: {:?}", table_description.table_status);
        }
    ...
    pub async fn add_vote_request(&self, signed_request: SignedVoteRequest) -> Result<(), Error> {
        ...
        self.client
            ...
            .item("GenesisHash", AttributeValue::S(format!("{:#x}", request.genesis_hash)))
            .item("PollIndex", AttributeValue::N(request.poll_index.to_string()))
            ...
            .item("Conviction", AttributeValue::S(format!("{:?}", request.conviction)))
            .item("SignedVoteRequest", AttributeValue::B(Blob::new(signed_request.encode())))
            .send().await?;
    ...
    pub async fn remove_vote_request(
        ...
        Ok(delete_item_output.attributes.filter(|attrs| !attrs.is_empty()).is_some())
    }

    pub async fn get_poll(&self, poll_index: u32) -> Result<Vec<SignedVoteRequest>, Error> {
        let signed_vote_requests = self.get_poll_items(poll_index, "SignedVoteRequest").await?
            .iter()
            .filter_map(|item| {
            ...
            .filter_map(|blob| {
                match SignedVoteRequest::decode(&mut blob.as_ref()) {
            ...
            .filter_map(|signed_request| {
                if signed_request.request.poll_index == poll_index {
                    ...
                    warn!("SignedVoteRequest is not for poll {}: {:?}", poll_index, signed_request);
                    None
                }
            })
            .collect::<Vec<_>>();
    ...
    pub async fn remove_poll(&self, poll_index: u32) -> Result<(), Error> {
        ...
        let account_values = self.get_poll_items(poll_index, &self.sort_key).await?
            .into_iter()
    ...
    pub async fn get_poll_indices(&self) -> Result<HashSet<u32>, Error> {
        ...
        loop {
            ...
            for item in scan_output.items() {
                let poll_index = item.get("PollIndex")
                ...
                if let (Some(poll_index), Some(account)) = (poll_index, account) {
                    vote_accounts.entry(poll_index)
                        .or_default()
                        .insert(account);
                } else {
...
fn is_attribute_type(
    ...
    let attribute_definition = AttributeDefinitionBuilder::default()
        ...
        .build()?;
    Ok(table_description.attribute_definitions().contains(&attribute_definition))
}
